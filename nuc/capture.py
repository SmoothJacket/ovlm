"""
Stereo frame capture using OpenCV VideoCapture.
...
def _make_camera(idx: int) -> cv2.VideoCapture:
    ...
    if config.GAIN_VALUE >= 0:
        cap.set(cv2.CAP_PROP_GAIN, float(config.GAIN_VALUE))
...
class StereoCapturer:
    ...
    def start(self) -> None:
        self._cap0 = _make_camera(config.CAM0_IDX)
        self._cap1 = _make_camera(config.CAM1_IDX)
        self._running = True
